...
date: 2025-09-14
description: "Thinking Machines 博客译文：LLM 推理非确定性的根源不是浮点非结合性，而是批次不变性缺失；逐层给出 RMSNorm、Matmul、Attention 的批次不变实现与实验验证。"
categories: ["translations"]
topics: ["llm"]
tags: ["llm", "inference", "thinking-machines"]
series: ["lab-blogs"]
series_order: 7
showAuthor: false
aliases: ["/blogs/llm/defeating-nondeterminism-in-llm-inference/"]
---
...
![image.png](https://cdn.jsdelivr.net/gh/gongzitaiyi/picture@master/uPic/2026/09/cover.svg)

{{< alert "claude" >}}
[阅读原文：Defeating Nondeterminism in LLM Inference（Thinking Machines）](https://thinkingmachines.ai/blog/defeating-nondeterminism-in-llm-inference/)
{{< /alert >}}
...
> 📊 原文此处为交互式可视化，演示两个不同指数的浮点数相加时的精度丢失：[在原文中查看](https://thinkingmachines.ai/blog/defeating-nondeterminism-in-llm-inference/)
...
> 🎬 原文此处为交互动画，演示原子加法归约的执行过程：[在原文中查看](https://thinkingmachines.ai/blog/defeating-nondeterminism-in-llm-inference/)
>
...
![image.png](https://cdn.jsdelivr.net/gh/gongzitaiyi/picture@master/uPic/2026/09/deterministic.svg)
...
![图4：虽然可以声称推理服务器本身是“确定性的”，但对于单个用户而言，情况就不同了。从单个用户的角度来看，其他并发用户不是系统的“输入”，而是系统的非确定性属性。这使得从每个用户的角度来看，LLM 推理具有“非确定性”。](https://cdn.jsdelivr.net/gh/gongzitaiyi/picture@master/uPic/2026/09/nondeterministic.svg)
...
![图5：数据并行 RMSNorm 理想情况下，我们希望避免并行化策略中内核之间的通信。实现这一目标的一种方法是将一个批处理元素分配给每个内核，从而保证每个归约完全在单个内核中完成。这被称为“数据并行”策略，因为我们只是沿着不需要通信的维度进行并行化。在这个例子中，我们有四行和四个内核，使我们的内核饱和。](https://cdn.jsdelivr.net/gh/gongzitaiyi/picture@master/uPic/2026/09/rmsnorm-01.svg)
...
![图6：用于更大批量的数据并行 RMSNorm 扩展到更大批量的数据并行策略非常简单 --- 不是让每个核心处理一行，而是允许每个核心依次处理不同的行。由于每个批处理元素的归约策略保持不变，因此这保留了批处理不变性。](https://cdn.jsdelivr.net/gh/gongzitaiyi/picture@master/uPic/2026/09/rmsnorm-02.svg)
...
![图7：拆分-减少RMSNorm 如果我们有一个小的批处理大小，我们的数据并行策略可能不再有足够的并行性来饱和我们的核心。在这种情况下，在多个核心之间“拆分”减少可能更有效，从而使我们能够充分利用我们的GPU。但是，这会失去批处理不变性，因为我们不再以相同的顺序减少每个元素。](https://cdn.jsdelivr.net/gh/gongzitaiyi/picture@master/uPic/2026/09/rmsnorm-03.svg)
...
与 RMSNorm 不同，围绕算术强度和利用 tensorcore 的额外约束迫使我们拆分 2D  tile ，而不是单个输出元素，以实现高效的 matmul 内核。](https://cdn.jsdelivr.net/gh/gongzitaiyi/picture@master/uPic/2026/09/matmul-01.svg)
...
![图9：Split-K Matmul 如果我们的批处理维度相当小，我们可能没有足够的并行性，需要一个 split-k matmul。在这个例子中，我们将每个归约分成两个核心，这两个核心将分别累积，然后在最后合并它们的结果。然而，将每个归约分成两个核心，使我们仍然可以利用八个核心。](https://cdn.jsdelivr.net/gh/gongzitaiyi/picture@master/uPic/2026/09/matmul-03.svg)
...
![图10：填充后的 Tensor-Core 指令。如果批量大小太小，我们可能无法在输出中放入任何一个 2D tile。在这种情况下，切换到较小的 tensor-core 指令或完全避开 tensor-core 是最有效的！然而，这两种选择都会阻止我们的内核保持批量不变性。](https://cdn.jsdelivr.net/gh/gongzitaiyi/picture@master/uPic/2026/09/matmul-02.svg)
...
> 📊 原文此处为交互式性能图表，展示批次不变 matmul 与 cuBLAS 的耗时对比：[在原文中查看](https://thinkingmachines.ai/blog/defeating-nondeterminism-in-llm-inference/)
>
...
![图12：FlashAttention2策略：我们沿着Q进行并行化，并同时沿着K/V进行归约。这意味着我们的整个归约可以保持在单个核心内，使其成为另一种数据并行策略。](https://cdn.jsdelivr.net/gh/gongzitaiyi/picture@master/uPic/2026/09/attention-01.svg)
...
例如，如果我们 KV 缓存中没有元素并且总共处理 128 个元素，那么在这两种情况下，我们都需要具有相同的数值，以确保注意力的“批次不变性”。](https://cdn.jsdelivr.net/gh/gongzitaiyi/picture@master/uPic/2026/09/attention-02.svg)
...
不幸的是，这也破坏了批次不变性，因为我们精确的规约策略取决于我们在任何给定请求中处理了多少来自序列的查询  token 。](https://cdn.jsdelivr.net/gh/gongzitaiyi/picture@master/uPic/2026/09/attention-03.svg)
...
这使我们能够保持批次不变性，因为我们的规约策略不再依赖于我们一次处理多少个查询  token ！](https://cdn.jsdelivr.net/gh/gongzitaiyi/picture@master/uPic/2026/09/attention-04.svg)
...
> 📊 原文此处为交互式图表，展示三种运行方式下采样器与训练器之间 logprobs KL 散度的变化曲线：[在原文中查看](https://thinkingmachines.ai/blog/defeating-nondeterminism-in-llm-inference/)
>
...
⁹ 可以在[这里](https://www.thonking.ai/p/what-shapes-do-matrix-multiplications)找到有关 LLM 推理中矩阵乘法形状的有趣分析。
