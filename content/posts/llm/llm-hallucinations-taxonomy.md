...
showAuthor: false
aliases: ["/blogs/llm/llm-hallucinations-taxonomy/"]
---
...
- **幻觉条件**: 一个LLM **h** 被认为相对于基准真相函数 **f** 产生了“幻觉”，如果在其所有训练阶段中，总存在至少一个输入字符串 **s**，使得LLM的输出 **h(s)** 与正确的输出 **f(s)** 不匹配。形式化表达为：\(\forall i \in \mathbb{N}, \exists s \in S\) 使得 \(h_i(s) \neq f(s)\)。
