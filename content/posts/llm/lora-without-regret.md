...
topics: ["llm"]
tags: ["llm", "training", "thinking-machines"]
series: ["lab-blogs"]
series_order: 9
showAuthor: false
aliases: ["/blogs/llm/lora-without-regret-thinkingmachines/"]
---
...
{{< alert "claude" >}}
[阅读原文：LoRA Without Regret — Thinking Machines](https://thinkingmachines.ai/blog/lora/)
{{< /alert >}}
...
相比之下，后训练涉及更小的数据集，通常聚焦于更狭窄的知识领域和行为范围。使用太比特（terabit）的权重来表示来自千兆比特（gigabit）或兆比特（megabit）训练数据的更新似乎是浪费的。这种直觉激发了参数高效微调（Parameter Efficient Fine-Tuning，PEFT），它通过更新一组更小的参数来调整大型网络。
...
- **多租户服务（Multi-tenant serving）**。由于LoRA在保持原始权重不变的同时只训练一个适配器（即A和B矩阵），单个推理服务器可以在内存中保留许多适配器（不同的模型版本）并以批处理方式同时从它们采样。[Punica: Multi-Tenant LoRA Serving](https://arxiv.org/abs/2310.18547)（Chen, Ye等，2023）现代推理引擎如vLLM和SGLang实现了此功能。
...
我们发现FullFT的最优学习率比高秩LoRA低10倍。参见Biderman等（2024）的图S1，他们对使用采样评估的实验发现了类似的10倍比率。我们将在稍后讨论LoRA超参数时回到这一点。

所有不同秩的LoRA运行的最优LR似乎相似；我们在下面为这一发现提供理论解释。然而，确实存在一些秩依赖性，Rank=1的最优LR低于更高秩LoRA。在Rank=4和Rank=512之间，最优LR变化不到2倍。

#### 批量大小效应

我们发现在某些设置中，LoRA对大批量大小的容忍度低于FullFT。性能差距随着更大的批量大小而增长，与秩无关。对于下一个实验，我们使用了 OpenThoughts3 的一个包含 10,000 个示例的小型子集。
...
右侧图表将最终损失绘制为批量大小的函数。我们看到，随着批量大小增大，LoRA与FullFT之间的损失差距越来越大。
...
对于这些实验，我们使用了一个基本的策略梯度算法，带有重要性采样修正；\(\text{objective} =\sum_t \frac{p_{\text{learner}}}{p_{\text{sampler}}} Adv_t\)（参见 [Your Efficient RL Framework Secretly Brings You Off-Policy RL Training](https://fengyao.notion.site/off-policy-rl)）。我们使用了类似GRPO的中心化方案[🔗](https://arxiv.org/abs/2402.03300)（Shao等，2024），其中我们为每个问题采样多个完成并减去每组的平均奖励。
...
- \(\text{init}_A / LR_A\)。由于Adam在每一步将 \(A\) 的元素更新约 \(LR_A\) ，这个时间尺度参数决定了让 \(A\) 显著偏离其初始状态所需的步数。
...
在我们的实验中，我们使用了Huggingface [`peft`](https://github.com/huggingface/peft)库的标准参数化（Mangrulkar等，2022），由Hu等提出：\(A\) 使用尺度为 \(1/\sqrt{d_{in}}\) 的均匀分布，\(B\) 零初始化，两者使用相同的LR，\(α = 32\)。在我们的调参过程中，未能找到比该组合更优的超参数。
...
这意味着对于较短的训练运行，最优LR应该设置得更高。初步证据表明，对于较短的运行，相对于FullFT的最优乘数约为15倍（基于传闻证据，更高的乘数在约100步以内有效）；对于更长的运行，则收敛到前述的10倍乘数。
...
如我们之前所示，如果我们仅将LoRA放在注意力层上，即使在数据量极小的场景下，学习也会更慢。
...
每个参数2比特的结果依赖于巧妙构建的合成数据集，以包含精确量的信息。估计给定现实学习问题所需的信息内容并不那么直接。一个经典观察是，当最小化对数损失时，在第一个epoch期间测量的总对数损失提供了数据集描述长度的测量。也就是说，它是记忆数据集所需比特数的一个上界。LLM数据集通常每个token的损失约为1比特（0.69 nats），具体取决于数据集和模型大小。
...
如果按FLOP（这种分析省略了注意力所需的FLOP，而它们在长上下文设置中可能很重要）而不是训练步数来绘制LoRA的性能，它将显示出相对于FullFT的明显优势。
