---
title: "Small Leak Can Sink a Great Ship—Boost RL Training on MoE with 𝑰𝒄𝒆𝑷𝒐𝒑!"
date: 2025-09-23
description: "IcePop 译文：MoE 模型 RL 训练中，训练与推理引擎的概率不匹配会复合放大直至崩溃；IcePop 用双侧掩码丢弃噪声梯度，保住稳定训练。"
categories: ["translations"]
topics: ["llm"]
tags: ["rl", "training", "llm"]
showAuthor: false
aliases: ["/blogs/llm/small-leak-can-sink-a-great-ship-boost-rl-training-on-moe-with-icepop/"]
---
...
{{< alert "claude" >}}
[阅读原文：Small Leak Can Sink a Great Ship — Boost RL Training on MoE with IcePop（InclusionAI / Ring 团队）](https://ringtech.notion.site/icepop)
{{< /alert >}}
...
- 一个重要的方向是正式表征 *崩溃边界*，即在线策略训练开始变得不稳定的临界点。
- 此外，IcePop 并未完全消除训练-推理不一致：训练与推理算子之间的底层数值差异仍是不稳定性的潜在来源，解决这一系统层面的根本问题是未来模型稳定扩展的关键。

> 译注：原文（notion 博客）结尾在此处缺失，「崩溃边界」一句的补全及以上要点依据论文版 [Ring-1T 技术报告（arXiv:2510.18855）](https://arxiv.org/abs/2510.18855) 第 5 节译出。
