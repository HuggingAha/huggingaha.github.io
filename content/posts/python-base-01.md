...
series_order: 1
showAuthor: false
aliases: ["/blogs/python/base/python-variables-operators-data-types/"]
---
...
- 其他运算符
    ...
    '''
    - is, is not 对比的是两个变量是否指向同一个对象（对象身份）
    - ==, != 对比的是两个变量的值
    - 即使两个变量指向不可变类型（如 str），is，is not 与 ==，!= 也不完全等价；小整数、短字符串等对象的缓存是 CPython 的实现细节，不能作为 Python 语言保证。
    - 对比的两个变量如果指向不同的可变对象（如 list、dict），即使它们的内容相同，is 和 == 的结果也可能不同。
    '''
