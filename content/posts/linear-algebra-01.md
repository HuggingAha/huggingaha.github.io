...
title: "线性代数：引言"
date: 2026-02-10
description: "从几何向量、多项式和信号理解向量的封闭性，建立向量空间直觉，并看清它为何是机器学习数据表示的基础。"
categories: ["notes"]
topics: ["math"]
tags: ["math"]
...
series_order: 1
showAuthor: false
aliases: ["/blogs/math/linear-algebra/01/"]
---
...
严格定义一个**向量空间**需要满足包含结合律、分配律等在内的 8 条公理（详见文末思考题）。其中，向量空间的集合需要对向量加法和数乘保持封闭：

1. **向量加法封闭** ：对任意 \(\boldsymbol{u}, \boldsymbol{v} \in V\)，都有 \(\boldsymbol{u}+\boldsymbol{v} \in V\)。
2. **数乘封闭** ：对任意 \(\boldsymbol{v} \in V\) 和标量 \(\lambda \in \mathbb{R}\)，都有 \(\lambda\boldsymbol{v} \in V\)。

这两条封闭性说明运算结果不会超出集合，但仅有封闭性还不足以构成向量空间，还需要满足其余公理。
...
> 向量空间的核心特征：其集合对向量加法和数乘保持封闭，且满足其余向量空间公理；集合中的元素才称为向量。
> 
...
- **可成为赋范空间** ：若赋予上确界范数 \(\|f\|_{\infty} = \sup_{x \in [a,b]} |f(x)|\)，\(C[a,b]\) 成为赋范空间。
...
- **向量的本质** ：向量是向量空间中的元素；向量空间的集合对向量加法和数乘封闭，并满足其余公理。
