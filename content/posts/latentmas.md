...
3.  **工程友好** ：不需要微调模型；支持 vLLM 部署，但由于无法直接注入非标准 KV Cache，需要采用 **Embedding 拼接 + 重新 Prefill** 的变通方案，并非完全即插即用。
