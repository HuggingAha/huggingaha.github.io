...
date: 2025-07-15
description: "源码分析医疗 Hybrid RAG 的混合检索、冲突检测和输出审计，理解如何为高风险问答建立安全闭环。"
categories: ["notes"]
topics: ["rag"]
tags: ["rag"]
showAuthor: false
aliases: ["/blogs/rag/hybrid-rag-system/"]
---
...
def _calculate_weights(self, query_analysis):
    ...
    if query_type == 'factual': # 事实类问题依赖语义
        ...
        weights['faiss'] = 0.8 # 注：低置信度时仅小幅调整，不会反转权重
        
    # 兜底机制：如果语义理解置信度低，适度降低语义权重、提高关键词权重
    if confidence < 0.3:
        weights['faiss'] *= 0.9
        weights['es'] *= 1.1
    return weights
