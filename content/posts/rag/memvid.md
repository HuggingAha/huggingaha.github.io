---
title: "Memvid：将海量文本压缩进视频的记忆库"
date: 2025-09-26
description: "拆解 Memvid 将文本编码为二维码视频的架构，理解视频压缩、FAISS 索引和 RAG 检索如何构成便携记忆库。"
categories: ["notes"]
topics: ["rag"]
tags: ["developer-tools"]
showAuthor: false
aliases: ["/blogs/project/memvid-compressing-massive-text-into-a-single-video-file/"]
---
...
<!-- # Memvid：将海量文本压缩进视频的记忆库 -->


## I. 项目概述

`Memvid` 是一个创新的AI记忆库项目。其核心思想是**将大规模文本知识库压缩成一个视频文件，并配套生成 JSON 元数据索引和 FAISS 向量索引** 。文本不是以普通文档形式保存，而是先被分块（Chunk），每一块内容编码成一个二维码（QR Code），并将这些二维码作为视频的连续帧（Frame），最终利用现代视频编码技术（如 H.265/HEVC）生成一个高度压缩的 `.mp4` 或 `.mkv` 文件。

这种设计的精妙之处在于，它巧妙地**将文本数据的压缩问题转化为了视频数据的压缩问题**，从而利用了数十年来在视频编码领域积累的先进技术。视频编码器尤其擅长压缩具有高度空间和时间冗余的图像序列，而由二维码组成的视频帧正是这种理想的压缩对象。

最终，`Memvid` 实现了一个类似于“AI记忆的SQLite”的解决方案：一个由视频文件和配套索引组成、自包含且无需复杂基础设施的便携式知识库。
...
此流程将原始文本数据转化为由一个视频文件和两个索引文件（`.json` 和 `.faiss`）组成的记忆库。
