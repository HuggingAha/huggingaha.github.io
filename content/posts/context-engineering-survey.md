---
title: "Context Engineering 综述解读"
date: 2025-07-19
description: "基于综述梳理 Context Engineering 的组件与系统分类，建立检索、记忆、工具及多 Agent 协同框架。"
categories: ["notes"]
topics: ["agent"]
tags: ["context-engineering", "agent"]
showAuthor: false
aliases: ["/blogs/agent/context-engineering-survey/"]
---
...
$$
P_{\theta}(Y|C) = \prod_{t=1}^{T} P_{\theta}(y_t|y_{\lt t}, C)
$$
...
| **目标** | \(\arg\max_{\text{prompt}} P_\theta(Y|\text{prompt})\) | \(F^* = \arg\max_F \mathbb{E}_{\tau \sim T} [\text{Reward}(P_\theta(Y|C_F(\tau)), Y^*_\tau)]\) |
