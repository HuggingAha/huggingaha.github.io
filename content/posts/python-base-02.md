...
series_order: 2
showAuthor: false
aliases: ["/blogs/python/base/python-sequence-container-types/"]
---
...
- 更新列表
    ...
    - `list.extend(iterable)` 将可迭代对象中的每个元素依次追加到列表末尾，直接修改原列表。
...
字典是键:值（`{key:value}`）对集合。Python 3.7+ 起，字典保持插入序（语言规范保证），键必须是互不相同的（在同一个字典之内）。
...
- 内置方法
    ...
    - `dict.popitem()` 在 Python 3.7+ 中按 LIFO 顺序返回并删除字典中最后插入的一对键值。如果字典已经为空，调用此方法会报出 `KeyError` 异常。
